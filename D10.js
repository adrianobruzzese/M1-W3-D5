...
const isThisAnEmail = function(string) {
  const regExpObject = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/
  if (regExpObject.test(string) === true){ return true }
  else {return false
}
}
...
const whatDayIsIt = function(){
  ...
  return `Oggi è ${today.toLocaleDateString('it-IT', {weekday: 'long'})} ${today.getDate()}`
}
...
// console.log(rollTheDices(1800000))
...
//   let today = new Date()
//   let checkToday = new Date(data)

// INCOMPLETO, non sono riuscito a capirlo. Sono sicuro di dover fare la differenza in ms, ma non riesco ad articolarlo in questo momento
...
let orderedMovies = movies.toSorted ((a,b) => a.Year - b.Year)
